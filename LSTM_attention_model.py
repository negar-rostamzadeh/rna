...
class LSTMAttention(BaseRecurrent, Initializable):
    def __init__(self, configs, **kwargs):
        ...
        cropper = LocallySoftRectangularCropper(
            ...
            hyperparameters={'cutoff': 30, 'batched_window': True},
            kernel=Gaussian())
        self.rescaling_factor = (float(self.patch_shape[0] /
                                 float(self.cropper_input_shape[0])))
        ...
        downn_sampled_inputs = cropper.apply(
            ...
            tensor.constant(
                ...
                [[float(
                    ...
                    self.cropper_input_shape[0]), ] * 2]).astype('float32'))
    ...
    def apply(self, inputs, states, location, scale, cells, mask=None):
        ...
        if self.num_channels == 3:
            ...
            gray_scale_inputs = inputs_normal
        ...
        patch = cropper.apply(
            ...
            np.array([list(self.cropper_input_shape)]),
            (location +
                tensor.ones_like(location)) * self.cropper_input_shape[0] / 2.0,
            (scale +
                tensor.ones_like(scale)) + self.min_scale)
        ...
        return (next_states, next_cells, location, scale, patch,
